...
const StackingSetting = ({
	...
	function handleAddTeam(e: MouseEvent<HTMLButtonElement>) {
		if (teamSelectRef.current) {
			...
			if (
				stacking[STACKING_SETTINGS.FROM_TEAMS]?.some(
					(team) => team === value
				)
			) {
				return;
			...
			const transformedTeams = stacking[STACKING_SETTINGS.FROM_TEAMS]
				? [...stacking[STACKING_SETTINGS.FROM_TEAMS], value]
				: [value];
	...
	function handleRemoveTeam(e: MouseEvent<HTMLButtonElement>) {
		...
		const transformedTeams = stacking[STACKING_SETTINGS.FROM_TEAMS].filter(
			(team) => team !== value
		);
	...
	return (
		<div>
			...
			{stacking[STACKING_SETTINGS.FROM_TEAMS]?.map((team) => (
				<button
